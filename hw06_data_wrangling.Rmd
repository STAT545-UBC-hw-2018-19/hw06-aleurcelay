...
The dataset `singer_locations` contains information about songs and associated artists in the [Million Song Dataset](https://labrosa.ee.columbia.edu/millionsong/).
...
The `singer_locations` dataframe contains geographical information associated with the artist location stored in two different formats: 1. as a (dirty!) variable named city; 2. as a latitude / longitude pair (stored in latitude, longitude respectively).
...
After, filtering the new dataframe `singer_geo` has 4129 observations, compared to 10100 that the original dataframe had.
However, as there are still so many observations, I will only work with the first 25 songs.
As there are many variables too, I will only keep a few to have an easier dataset to look at.
...
Now `singer_address` contains the coressponding addresses from the given coordinates.
...
I will try this again, puting all in low case.
